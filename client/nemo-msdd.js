/**
 * NeMo MSDD режим — JS такой же как у pyannote (общая логика модалки/прогресса),
 * но передаётся mode=nemo-msdd, и для задач показывается отдельный список.
 *
...
const API = {
    ...
    list:   () => apiFetch("/api/jobs?mode=nemo-msdd").then(r => r.json()),
